...
## [0.2.0](https://github.com/OxideAV/oxideav-celt/compare/v0.1.10...v0.2.0) - 2026-07-30
...
* **Round-389 (2026-07-04) — stereo PCM codec loop
  ...
  per-channel front end (`StereoPcmAnalysis`: §4.3.7.2 pre-emphasis
  FIR tap + §4.3.7 forward-MDCT history per channel) and the shared
  two-channel §4.3.2.1 coarse prediction;
  `encode_stereo_celt_frame_pcm_auto` →
  `StereoCeltDecodeState::decode_stereo_frame_auto` is a fully
  self-contained **interleaved stereo PCM → bytes → PCM** codec loop
  (one frame of algorithmic delay per channel, no out-of-band data).
  Rejections (length / transient / post-filter / under-budget dual
  gate) leave the streaming front end untouched.
  `tests/stereo_pcm_codec_loop.rs` pins the loop at every `LM`
  (coarse lockstep + decoded PCM ≡ synthesis of the encoder's
  bit-exact per-channel reconstructions), per-channel waveform
  fidelity on a distinct-tone stereo signal (relative L2 < 0.8,
  correlation > 0.6 per channel — channels unswapped), a stereo
  silence run decaying to numerical quiet in lockstep, byte
  determinism + reset, and the explicit-allocation round trip.
  +5 integration tests.
...
## [0.1.10](https://github.com/OxideAV/oxideav-celt/compare/v0.1.9...v0.1.10) - 2026-07-03
